...
# Streamlit Cloud runs headless; force the non-interactive backend BEFORE
# importing pyplot so matplotlib never tries to touch a display.
import matplotlib
matplotlib.use("Agg")
...
try:
    ...
    st.error(
        ...
        "This usually means `requirements.txt` isn't being picked up, or the "
        "Streamlit Cloud build cache is stale.\n\n"
        "**Fix steps:**\n"
        "1. Confirm `requirements.txt` sits at the **repo root**, next to `app.py` "
        "(not inside a subfolder).\n"
        "2. On Streamlit Cloud: **Manage app → ⋮ menu → Reboot app** "
        "(a normal redeploy can reuse a broken cache).\n"
        "3. If that fails: **Manage app → ⋮ menu → Delete app**, then redeploy fresh "
        "from the same repo.\n\n"
